...
The same V2 run has passed the frozen step50 health node (3,200 examples seen):
projector/receiver RMS ratios are `1.0059/1.0727`, spread ratios
`0.9502/0.9466`, CE is `3.5188`, and the full projector/optimizer/RNG checkpoint
...
The public `inference/model.py` still exposes a text-only `forward(input_ids, start_pos=0)` and has no image-to-embedding injection or vision/projector symbols; public `config.json` has no `vision_config`. The most defensible status is therefore “multimodal tokenizer seam, no public vision runtime,” with private historical multimodal training unresolved.

### Qwen3.5-4B MoonViT V1/V2 regression

An execution audit found that the first V1 run used only indices `0–7`, while the frozen contract and V2 reference used `0–31`. The old result is preserved as an eight-row pilot. The preregistered repair reran both V1 arms with all 32 ordered records. Full32 CE-only vision/blind/shuffled click was `2%/2%/2%`; paired-margin was `2%/2%/0%`. The paired-margin vision-minus-blind CI was `[-6,+6] pp` and vision-minus-shuffled `[0,+6] pp`. The matched full32 V2 reference remains `4%/2%/4%` with no positive shuffle lower bound. V1 therefore still does not rescue grounding, now under a genuinely matched budget. Raw compact artifacts and the independent verifier are under `experiments/qwen3b_community_eval_20260805/capacity_controls/`.
## Regression matrix handoff (2026-08-08)
...
Kimi K2.6/MoonViT-3d 版本已完成真实单图 forward 和 50 条固定 ScreenSpot cache：输出 `(3354,4,1152)`、FP16 RMS 约 3.04、50/50 cache rows、0 failures。loader qkv 参数修复已推送为 `a6b4ee1`。Qwen2.5-7B 的 6,400 条 K26 训练 cache 已完成（6,400/6,400、0 failures、5,396 unique images），但 projector-only 100-step arm 在 step13 触发表征塌缩止损：relative spread step1=`0.092`、后续约 `0.03--0.04`，projector RMS `0.20→4.34`。因此没有运行能力生成评测，矩阵行记录为 `failed_evaluation`，完整 `train_health.jsonl`、训练日志和最近健康 checkpoint 已放入 immutable failure artifact `qwen25_7b_k26_moonvit3d_1152_projector_short_probe_attempt1/`。这回答了“接口能否识图”边界：视觉塔能出特征，但该 Qwen projector 轨迹没有证明识图。

## Public GLM-5.2V / WebBrain audit (2026-08-08)

The public releases show that others are ahead of this repository in target-specific packaging, but not yet in our causal evaluation contract. [Baseten GLM-5.2V](https://huggingface.co/baseten/GLM-5.2-Vision-NVFP4) uses frozen Kimi K2.6 MoonViT-3d (27 layers, 1152-d) and frozen GLM-5.2 with a trained ~49.5M PatchMerger projector; the accompanying [training report](https://www.baseten.co/blog/glm-52-with-vision/) describes 66k image-QA pairs, global batch 64, constant `5e-4`, two epochs and later projector-only reasoning RL, with MMMU-Pro 55% reported. It does not publish our ScreenSpot vision/blind/shuffled paired CIs or complete TextVQA/DocVQA/OCRBench table.

[WebBrain's DeepSeek-V4-Flash-0731-Vision package](https://huggingface.co/webbrain-one/DeepSeek-V4-Flash-0731-Vision-NVFP4) publishes a trained 40,119,040-parameter 4096-wide projector, Kimi tower and SGLang glue. Its own manifest still says `gpu_validated_for_this_0731_package=false`, with no public fixed causal benchmark. The projector shape matches our K26 Qwen projector, so the important difference is the DeepSeek bridge: OOV image sentinel `129280` plus a deterministic 64-ID routing palette during prefill. Our current DeepSeek merge repeats the placeholder ID and lacks that palette bridge. Treat WebBrain as a high-priority interface reference, not as a validated capability baseline or as a Qwen result.

Next action before any paid hardware: add an optional DeepSeek-only palette-cycle/OOV-sentinel bridge, then run the local tiny Gate D verifier (text forward, four-condition step0, finite projector input-gradient, checkpoint round-trip, and greedy image smoke). Do not claim Gate D or download/run the full assembled 0731 package until its validation boundary is independently passed.
## External discussion status (2026-08-08)

Discussion recorded: https://huggingface.co/webbrain-one/DeepSeek-V4-Flash-0731-Vision-NVFP4/discussions/3

The project must use the following wording: our community-comparable matrix is complete, but no external MoonViT projector arm has passed the causal capability gate. Kimi K2.6/MoonViT-3d feature extraction and 50-image caching passed only as an interface test. Qwen2.5-7B V1/V2/K26 and the matched top-layer LoRA probe did not show correct-image superiority over blind and shuffled; the K26 arm stopped for representation collapse before generation. The native Qwen VLM is an independent positive control, not an external MoonViT result. DeepSeek full-weight image forward/backward, FP4/FP8 dgrad, checkpoint round-trip and generation remain unrun; Gate D is 'NO-GO'.

For WebBrain, record only that public projector/tower artifacts and provenance exist. Its manifest says 'gpu_validated_for_this_0731_package=false', and no public fixed causal benchmark or training trace was found. It is valid to request reproducible validation evidence; it is not valid to assert fabrication or completed visual ability without evidence. The machine-readable status is experiments/external_model_audits/webbrain_discussion3_status_20260808.json.
