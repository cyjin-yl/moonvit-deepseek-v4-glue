...
Under the same frozen Qwen3.5 receiver, 32-sample projector-only budget, 16-token mean-pool interface and 50-row ScreenSpot contract, the V1 family proxy was retrained with CE-only and paired-margin controls. V1 CE-only reached 0% vision click-in-box; V1 paired-margin reached 2% versus 2% blind and 2% shuffled. Its paired vision-minus-blind CI was `[-6,+6] pp` and vision-minus-shuffled CI `[0,0] pp`. The matched V2 paired-margin reference was 4% vision, 2% blind and 4% shuffled, also without a positive causal lower bound. V1 therefore does not rescue external MoonViT grounding; the version-only hypothesis is rejected as the primary explanation. Raw compact artifacts and the independent verifier are under `experiments/qwen3b_community_eval_20260805/capacity_controls/`.
