...
Example::
    ...
    python tools/build_train_mix.py \
        ...
        --eval-images data/eval/images --out data/train_v1
"""
...
def hash_eval_images(eval_dirs: list[Path]) -> list[int]:
    hashes = []
...
def main() -> None:
    ...
    eval_hashes = hash_eval_images(args.eval_images)
    print(f"eval images hashed: {len(eval_hashes)} (threshold {args.threshold})")
    ...
    for spec in args.source:
        ...
        print(f"{name}: {len(rows)} rows -> kept {kept} (dropped {dropped})", flush=True)
